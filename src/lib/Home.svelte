...
<section class="section">
  <article class="message">
    ...
    <p class="mb-4">
      <strong><a href="https://github.com/Niek/chatgpt-web"
      target="_blank">TritonGPT-LLM</a></strong>
      is a simple one-page web interface to the TritonGPT Llama LLM.  All messages are stored in your browser's local storage, so everything is
  ...
  <article class="message" class:is-danger={!hasModels} class:is-warning={!apiKey} class:is-info={apiKey}>
    <div class="message-body">
      Set your TritonGPT LLM API key below:
      ...
      {#if !apiKey}
        <p class:is-danger={!hasModels} class:is-warning={!apiKey}>
          Please enter your TritonGPT-LLM API key above.
          At least one API must be enabled to use TritonGPT-LLM.
        </p>
      {/if}
    </div>
  </article>
